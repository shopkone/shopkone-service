...
func (s *sCustomerAddress) Add(customerId uint, address mAddress.Address) (id uint, err error) {
	// 查询用户中是否存在默认地址
	...
	err = s.orm.Select("is_default", "customer_id", "address_id", "shop_id").Create(&customerAddress).Error
	return customerAddress.AddressID, err
}
