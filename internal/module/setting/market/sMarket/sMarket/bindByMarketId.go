...
// 根据market_id绑定语言
func (s *sMarket) BindLangByMarketId(in *vo.BindLangByMarketIdReq) (err error) {
	// 过滤一下，只绑定是这个店铺的语言
	in.LanguageIDs, err = sLanguage.NewLanguage(s.orm, s.shopId).LanguageFilter(in.LanguageIDs)
	if err != nil {
		return err
	}
	...
	if err = s.orm.Model(&mMarket.MarketLanguage{}).
		Where("shop_id = ? AND is_default = ? AND market_id = ?", s.shopId, true, in.MarketID).
